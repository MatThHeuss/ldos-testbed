...
def main():
    ...
    ap.add_argument("--sweep", default="T", choices=["T", "l", "grid", "none"],
    ...
    T_VALUES = [0.30, 0.45, 0.50, 0.55, 0.75, 1.00, 1.10, 1.50]
    L_VALUES = [0.12, 0.24, 0.48]
    D_VALUES = [0.05, 0.10, 0.20]

    if args.sweep == "grid":
        # eixos livres: duty cycle D e periodo T; a duracao do pulso e derivada
        # por l = D * T, o que garante D < 1 e mantem a taxa media do atacante
        # em D fracao da capacidade do enlace.
        sweeps = []
        for dv in D_VALUES:
            for tv in T_VALUES:
                sweeps.append((("T", tv), ("l", round(dv * tv, 6)), ("__D", dv)))
    else:
        sweeps = {
            "T": [(("T", v),) for v in T_VALUES],
            "l": [(("l", v),) for v in L_VALUES],
            "none": [((None, None),)],
        }[args.sweep]
    ...
    for combo in sweeps:
        for rep in range(args.repeats):
            ...
            for (param, val) in combo:
                if param == "__D":
                    parts.insert(0, f"D{val}")
                elif param:
                    p[param] = val
                    parts.append(f"{param}{val}")
            p["seed"] = DEFAULTS["seed"] + rep
            tag = ("__".join(parts) if parts else "baseline")
            if args.repeats > 1:
                tag += f"__rep{rep}"
